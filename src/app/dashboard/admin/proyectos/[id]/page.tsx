...
import ListaView from "./_components/ListaView";
...
const COL_KEYS = ["por_hacer", "en_progreso", "en_revision", "completado"];

type View = "kanban" | "lista" | "calendario" | "gantt";
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ width: 13, height: 13 }}>
        ...
        <rect x="5" y="4" width="8" height="4" rx="1" fill="currentColor" stroke="none" opacity="0.5" />
        <rect x="10" y="10" width="9" height="4" rx="1" fill="currentColor" stroke="none" opacity="0.5" />
        <rect x="4" y="16" width="6" height="4" rx="1" fill="currentColor" stroke="none" opacity="0.5" />
      </svg>
    ),
  },
];
...
// ── Modal genérico ────────────────────────────────────────────────
function AppModal({ open, onClose, title, children }: { open: boolean; onClose: () => void; title: string; children: React.ReactNode }) {
  return (
        ...
        <div style={{ position: "fixed", inset: 0, overflowY: "auto" }}>
          <div style={{ display: "flex", minHeight: "100%", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
            <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-150" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <Dialog.Panel style={{ width: "100%", maxWidth: "460px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "16px", padding: "1.75rem", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
                  <Dialog.Title style={{ fontSize: "0.95rem", fontWeight: 700, color: "var(--text-primary)", margin: 0 }}>{title}</Dialog.Title>
...
// ── Confirm dialog ────────────────────────────────────────────────
function ConfirmDialog({ open, onClose, onConfirm, title, message, danger = true }: { open: boolean; onClose: () => void; onConfirm: () => void; title: string; message: string; danger?: boolean }) {
  return (
    <Transition appear show={open} as={Fragment}>
          ...
          <Transition.Child as={Fragment} enter="ease-out duration-150" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-100" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
            <Dialog.Panel style={{ width: "100%", maxWidth: "380px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "14px", padding: "1.5rem", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: "0.85rem", marginBottom: "1.1rem" }}>
                ...
                <div>
                  <Dialog.Title style={{ fontSize: "0.92rem", fontWeight: 700, color: "var(--text-primary)", margin: "0 0 0.3rem" }}>{title}</Dialog.Title>
                  <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", margin: 0, lineHeight: 1.5 }}>{message}</p>
                </div>
              </div>
              <div style={{ display: "flex", gap: "0.65rem", justifyContent: "flex-end" }}>
                <button onClick={onClose} style={{ padding: "0.5rem 1rem", borderRadius: "8px", border: "1px solid var(--border)", background: "transparent", cursor: "pointer", fontSize: "0.8rem", color: "var(--text-secondary)", fontWeight: 500 }}>Cancelar</button>
                <button onClick={() => { onConfirm(); onClose(); }} style={{ padding: "0.5rem 1rem", borderRadius: "8px", border: "none", background: danger ? "#ef4444" : "var(--verde)", color: "#fff", cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}>Eliminar</button>
              </div>
            </Dialog.Panel>
          </Transition.Child>
        </div>
...
export default function ProyectoPage() {
  ...
  const [tareaFrm, setTareaFrm] = useState({ titulo: "", descripcion: "", prioridad: "media", fechaInicio: "", fechaLimite: "", responsablesIds: [] as string[] });
  ...
  function handleDragStart(event: DragStartEvent) {
    ...
    origColRef.current = findColKeyFresh(tareaId);
    for (const impl of implRef.current) {
      const t = impl.tareas?.find((t: any) => t.id === tareaId);
  ...
  function handleDragOver(event: DragOverEvent) {
    ...
    const targetCol = COL_KEYS.includes(overId) ? overId : findColKeyFresh(overId);
    if (!targetCol || findColKeyFresh(tareaId) === targetCol) return;
    setImplementaciones((prev) =>
      prev.map((impl) => ({ ...impl, tareas: impl.tareas?.map((t: any) => t.id === tareaId ? { ...t, columna: targetCol } : t) }))
    );
  }
  ...
  async function handleCrearFase(e: React.FormEvent) {
    ...
    try {
      ...
      cargar();
    } finally { setFaseLoading(false); }
  }

  function handleEliminarFase(implId: string) {
    openConfirm("Eliminar fase", "Se eliminarán esta fase y todas sus tareas. Esta acción no se puede deshacer.", async () => {
  ...
  function abrirModalTarea(implId: string, colKey: string) {
    ...
    setTareaFrm({ titulo: "", descripcion: "", prioridad: "media", fechaInicio: "", fechaLimite: "", responsablesIds: [] });
    setModalTarea(true);
  }
  ...
  async function handleCrearTarea(e: React.FormEvent) {
    ...
    try {
      await crearTarea({
        ...
        fechaInicio: tareaFrm.fechaInicio || undefined,
  ...
  function handleEliminarTarea(tareaId: string) {
    openConfirm("Eliminar tarea", "¿Seguro que quieres eliminar esta tarea? Esta acción no se puede deshacer.", async () => {
      try {
        await eliminarTarea(tareaId);
        setImplementaciones((prev) => prev.map((impl) => ({ ...impl, tareas: impl.tareas?.filter((t: any) => t.id !== tareaId) })));
      } catch {}
    });
  }
  ...
  async function handleEditarTarea(e: React.FormEvent) {
    ...
    } finally { setEditTareaLoading(false); }
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "100%" }}>
      ...
      <div style={{ borderBottom: "1px solid var(--border)", paddingBottom: "1.25rem", display: "flex", flexDirection: "column", gap: "0.85rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
          <button
            onClick={() => router.back()}
            style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "30px", height: "30px", borderRadius: "8px", flexShrink: 0, background: "var(--bg-soft)", border: "1px solid var(--border)", cursor: "pointer", color: "var(--text-muted)", transition: "all 0.15s" }}
          ...
          {proyecto?.cliente && (
            <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
              ...
              <span style={{ fontSize: "0.76rem", color: "var(--text-muted)" }}>
                ...
                {proyecto.cliente.empresa && <span> · {proyecto.cliente.empresa}</span>}
              </span>
            </div>
          )}
        ...
        {VIEWS.map((v) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => { if (!active) { e.currentTarget.style.borderColor = "var(--verde)"; e.currentTarget.style.color = "var(--verde)"; } }}
      ...
      {view === "kanban" && (
        ...
        >
          <KanbanView
            ...
            onAgregarTarea={abrirModalTarea}
            onModalFase={() => setModalFase(true)}
          />
        </DndContext>
      )}
      ...
      <AppModal open={modalFase} onClose={() => setModalFase(false)} title="Nueva fase">
        <form onSubmit={handleCrearFase} style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            ...
            <input required placeholder="Ej: Diseño UI, Desarrollo backend..." value={faseFrm.nombre} onChange={(e) => setFaseFrm((p) => ({ ...p, nombre: e.target.value }))} style={inputS} />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
      ...
      {/* ── Modal nueva tarea ── */}
      <AppModal open={modalTarea} onClose={() => setModalTarea(false)} title="Nueva tarea">
        <form onSubmit={handleCrearTarea} style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            ...
            <input required placeholder="Título de la tarea..." value={tareaFrm.titulo} onChange={(e) => setTareaFrm((p) => ({ ...p, titulo: e.target.value }))} style={inputS} />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-secondary)" }}>Descripción</label>
            <textarea placeholder="Descripción opcional..." value={tareaFrm.descripcion} onChange={(e) => setTareaFrm((p) => ({ ...p, descripcion: e.target.value }))} rows={2} style={{ ...inputS, resize: "vertical" }} />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              ...
              <input type="date" value={tareaFrm.fechaInicio} onChange={(e) => setTareaFrm((p) => ({ ...p, fechaInicio: e.target.value }))} style={inputS} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              ...
              <div style={{ border: "1px solid var(--border)", borderRadius: "8px", background: "var(--bg-soft)", padding: "0.4rem 0", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                {proyecto.encargados.map((enc: any) => (
                  <label key={enc.id} style={{ display: "flex", alignItems: "center", gap: "0.55rem", padding: "0.35rem 0.75rem", cursor: "pointer", fontSize: "0.78rem", color: "var(--text-primary)", userSelect: "none" }}>
                    <input type="checkbox" checked={tareaFrm.responsablesIds.includes(enc.id)}
                      onChange={() => setTareaFrm((p) => ({ ...p, responsablesIds: p.responsablesIds.includes(enc.id) ? p.responsablesIds.filter((x) => x !== enc.id) : [...p.responsablesIds, enc.id] }))}
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.25rem" }}>
            <button type="button" onClick={() => setModalTarea(false)} style={{ flex: 1, padding: "0.55rem", borderRadius: "8px", border: "1px solid var(--border)", background: "transparent", cursor: "pointer", fontSize: "0.8rem", color: "var(--text-secondary)" }}>Cancelar</button>
      ...
      <AppModal open={modalEditTarea} onClose={() => setModalEditTarea(false)} title="Editar tarea">
        <form onSubmit={handleEditarTarea} style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-secondary)" }}>Fecha inicio</label>
              <input type="date" value={editTareaFrm.fechaInicio} onChange={(e) => setEditTareaFrm((p) => ({ ...p, fechaInicio: e.target.value }))} style={inputS} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              <label style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-secondary)" }}>Fecha límite</label>
          ...
          {proyecto?.encargados?.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              ...
              <div style={{ border: "1px solid var(--border)", borderRadius: "8px", background: "var(--bg-soft)", padding: "0.4rem 0", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                {proyecto.encargados.map((enc: any) => (
                  <label key={enc.id} style={{ display: "flex", alignItems: "center", gap: "0.55rem", padding: "0.35rem 0.75rem", cursor: "pointer", fontSize: "0.78rem", color: "var(--text-primary)", userSelect: "none" }}>
                    <input type="checkbox" checked={editTareaFrm.responsablesIds.includes(enc.id)}
                      onChange={() => setEditTareaFrm((p) => ({ ...p, responsablesIds: p.responsablesIds.includes(enc.id) ? p.responsablesIds.filter((x) => x !== enc.id) : [...p.responsablesIds, enc.id] }))}
                      style={{ accentColor: "var(--verde)", width: "14px", height: "14px" }} />
                    {enc.nombre}
                  </label>
                ))}
              ...
              <div style={{ border: "1px solid var(--border)", borderRadius: "8px", background: "var(--bg-soft)", maxHeight: "110px", overflowY: "auto", padding: "0.4rem 0", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                {subadmins.map((s: any) => (
                  <label key={s.id} style={{ display: "flex", alignItems: "center", gap: "0.55rem", padding: "0.35rem 0.75rem", cursor: "pointer", fontSize: "0.78rem", color: "var(--text-primary)", userSelect: "none" }}>
                    <input type="checkbox" checked={editProyFrm.encargadosIds.includes(s.id)}
                      onChange={() => setEditProyFrm((p) => ({ ...p, encargadosIds: p.encargadosIds.includes(s.id) ? p.encargadosIds.filter((x) => x !== s.id) : [...p.encargadosIds, s.id] }))}
                      style={{ accentColor: "var(--verde)", width: "14px", height: "14px" }} />
                    {s.nombre}
                  </label>
                ))}
